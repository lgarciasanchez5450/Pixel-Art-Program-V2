...
import pickle
canvas:Canvas
save_as_png_name:str = ''
save_AS_GIF_name:str = ''
GIF_fps:int = 30
GIF_pixel_size:int = 1
PNG_pixel_size = 1
h,s,v = (0,0,0)
name_alert_time:float = 0.0
canvas_width:int = 64
canvas_height:int = 64
saved_project_names:list[str] = findAllFiles('.bin','/__Projects/')
...
def set_color_brick_h(_h):
  ...
  window_space.miniWindow('Color Picker').color_brick.color = color
  color_pure = hsv_to_rgb(h,1,1)
  print(color_pure)
  window_space.miniWindow('Color Picker').sat_bg.image.fill(color_pure)
  color_pure = hsv_to_rgb(h,s,1)
...
def backup_project(save_as) -> None:
  ...
  #store_them_in_file
  with open(PATH+'/Backups/'+save_as+'.bin','wb') as file:
...
def save_project() -> None:
  ...
  palette_info = color_palette.get_info()
  #store_them_in_file
  with open(PATH+'/__Projects/'+canvas.name+'.bin','wb') as file:
    pickle.dump((canvas_info,palette_info),file) 
  canvas.saved = True
def maybe_quit() -> None:
  if canvas.saved:
...
def load_project(fileName:str) -> None:
  ...
  with open(PATH+'/__Projects/'+fileName+'.bin','rb') as file:
