...
class Canvas:
    ...
    def make_active_frame_copy(self):
        ...
        self.grids[-1].grid = deepcopy(self.grids[self.active_grid].grid)
    ...
    def set_current_tool(self,tool) -> None:
        assert isinstance(tool,(int, str)), f'Cannot set {tool} as a tool'
        try:
    ...
    def update_mask(self):
        ...
        for count in range(self.grid_size[1]): #horizontal lines
            line(self.grid_surf,(0,0,0,255),(0,count*self.screen_size),(surf_width,count*self.screen_size),self.screen_size//15)# '15' is for how thin the grid_lines are compared to the cells
    ...
    def pen_draw_pixel(self,x,y,color) -> None:
        self.current_grid.line((self.px,self.py),(x,y),color)
    ...
    def enlarge_canvas(self,newSize) -> None:
        ...
        self.pos = (round(self._pos[0]-diff_x*previous_mousex_percent),round(self._pos[1]-diff_y*previous_mousey_percent))
        ...
        self.pixel_rect = Rect(0,0,newSize,newSize)
    ...
    def update(self,things):
        ...
        if self._rect.collidepoint(mpos) or self._rect.collidepoint(self.pmpos):
            if wheel:
                ...
                self.pos = (self._pos[0]+mpos[0]-self.pmpos[0],self._pos[1]+mpos[1]-self.pmpos[1])
            elif self.tool == self.tools.index('pen') and mb1 and self.current_color:
                #print(type(mpos[0]),type(self._pos[0]),type(self._offSetPos[0]),type(self.screen_size))
                x,y = (mpos[0]-self._pos[0]-self._offSetPos[0])//self.screen_size,(mpos[1]-self._pos[1]-self._offSetPos[1])//self.screen_size
                if mb1down:
                    self.px,self.py = x,y           
                    self.saved = False         
                print(x,y)
                ...
                for x,y in self.current_grid.flood_exact_list((int((mpos[0]-self._pos[0]-self._offSetPos[0])//self.screen_size),int((mpos[1]-self._pos[1]-self._offSetPos[1])//self.screen_size)),self.current_color):
                ...
                x,y = int((self.mpos[0]-self._pos[0]-self._offSetPos[0])//self.screen_size),int((mpos[1]-self._pos[1]-self._offSetPos[1])//self.screen_size)
                if mb1down:
                    self.px,self.py = x,y                    
...
class Palette:
    ...
    def add_color(self,color:tuple):
        assert color!=None, "Cannot add color 'None' to Palette"
        assert not isinstance(color, (int,float,str,bytes,bool)), f'Type {type(color)} cannot be a color'
        if color not in self.colors and len(self.colors)-1 < self.max:
            ...
            return
        #if interpreter reaches here it means that it
        #could not add color, either the palette was 
        #full or color already in palette

    def select_color(self,color:tuple):
        assert (color in self.colors), f'Color {color} not in palette!'
...
class Grid:
    ...
    def flood_recursive(self,starting_point,color):
        ...
        try:
            ...
            raise SyntaxError("Should not use recursive flood fill")
...
if __name__ == "__main__":
    ...
    def expensive_func():
        ...
        return True
    func_alias = func
    expensive_func_alias = expensive_func
    ...
    for x in range(z):
        ...
        pure_func.append((end-start)/100)
        a = 0
        start = perf_counter()
        for q in my_list:
            func_alias()
        end = perf_counter()
        alias_func.append((end-start)/100)
    print('pure_func Time Avg:', sum(pure_func)/len(pure_func))
    print('alias_func Time Avg:', sum(alias_func)/len(alias_func))
    plt.plot(list(range(z)),pure_func,'b-') #blue is pure_func   
    plt.plot(list(range(z)),alias_func,'r-') #red is alias_func
    plt.show()
